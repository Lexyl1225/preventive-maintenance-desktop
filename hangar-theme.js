...
*/
  ...
  function observeDOM(){
    ...
    var observer = new MutationObserver(function(){
      // Debounce: batch rapid DOM changes into a single rAF callback
      if(!pending){
        pending = true;
  ...
  function init(){
    ...
    // Skip parallax background on mobile (mousemove doesn't fire + saves a compositing layer)
    if(!mobile){
    ...
    // Reduce dots on mobile (fewer compositing layers); CSS hides them anyway but skip DOM creation
    if(!mobile){
      createDots(8);
    }
